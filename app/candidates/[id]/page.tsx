...
export default function CandidateDetail({ params }: PageProps) {
  ...
  const handleAddNote = async (noteData: { content: string; mentions: TeamMember[] }) => {
    try {
      await DatabaseService.createNote({
        content: noteData.content,
        candidateId: id,
        mentions: noteData.mentions.map((m) => m.id),
        tags: [],
      });
        ...
        <div className="rounded-xl border bg-card shadow-sm">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-5">
            <div className="flex items-start gap-4">
              <Link href="/candidates" className="shrink-0">
          ...
          <div className="border-t p-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              ...
              <InfoRow
                icon={ClockIcon}
                label="Current Stage"
                value={STAGE_LABELS[candidate.stage]}
              />
            </div>
          </div>
        </div>
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={async (newStage) => {
                      try {
                        await DatabaseService.updateCandidate(id, {
      ...
      >
        <ModalBody>
          <p className="text-muted-foreground">
            ...
            cannot be undone and will also delete all associated notes and timeline
            events.
          </p>
        </ModalBody>
        <ModalFooter>
...
function CandidateEditForm({ candidate, onSubmit, onCancel }: CandidateEditFormProps) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <Select
          ...
          onValueChange={(value) =>
            setFormData((s) => ({ ...s, stage: value as CandidateStage }))
          }
        >
          <SelectTrigger>
            <SelectValue placeholder="Select stage" />
...
interface NoteFormProps {
  onSubmit: (data: { content: string; mentions: TeamMember[] }) => void;
  onCancel: () => void;
}
...
function NoteForm({ onSubmit, onCancel }: NoteFormProps) {
  ...
  const extractMentions = (text: string): TeamMember[] => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          Tip: Type @ followed by a name to mention team members. They&apos;ll be
          notified about this note.
        </p>
      </div>
