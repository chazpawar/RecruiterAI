...
export default function AssessmentEditPage({ params }: PageProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const assessmentData = await DatabaseService.getAssessmentById(assessmentId);
      if (!assessmentData) {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-6">
        <EmptyState
          ...
          subtitle="The requested assessment does not exist or has been removed."
          action={
  ...
  const headerSubtitle = `Edit assessment${job ? ` for ${job.title}` : ''}`;
