...
function TypeBadge({ type }: TypeBadgeProps) {
  const map: Record<QuestionType, { label: string; cls: string }> = {
    [QUESTION_TYPES.SINGLE_CHOICE]: {
      label: 'Single Choice',
      cls: 'bg-blue-50 text-blue-700 dark:bg-blue-900/25 dark:text-blue-300',
    },
    [QUESTION_TYPES.MULTI_CHOICE]: {
      label: 'Multiple Choice',
      cls: 'bg-green-50 text-green-700 dark:bg-green-900/25 dark:text-green-300',
    },
    [QUESTION_TYPES.SHORT_TEXT]: {
      label: 'Short Text',
      cls: 'bg-yellow-50 text-yellow-700 dark:bg-yellow-900/25 dark:text-yellow-300',
    },
    [QUESTION_TYPES.LONG_TEXT]: {
      label: 'Long Text',
      cls: 'bg-purple-50 text-purple-700 dark:bg-purple-900/25 dark:text-purple-300',
    },
    [QUESTION_TYPES.NUMERIC]: {
      label: 'Numeric',
      cls: 'bg-orange-50 text-orange-700 dark:bg-orange-900/25 dark:text-orange-300',
    },
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${meta.cls}`}
    >
      {meta.label}
    </span>
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-2">
        ...
        <Select
          value={formData.type}
          onValueChange={(value) => setFormData((s) => ({ ...s, type: value as QuestionType }))}
        >
          <SelectTrigger>
...
function AssessmentPreview({ assessment }: AssessmentPreviewProps) {
  return (
            ...
            {section.questions.map((q, qIdx) => (
              <div key={q.id}>
                ...
                {(q.type === QUESTION_TYPES.SINGLE_CHOICE ||
                  q.type === QUESTION_TYPES.MULTI_CHOICE) && (
                  <div className="mt-3 space-y-2">
                    {q.options?.map((opt, i) => (
                      <label key={i} className="flex items-center gap-2 text-sm">
                        <input
                          disabled
                          type={q.type === QUESTION_TYPES.SINGLE_CHOICE ? 'radio' : 'checkbox'}
                        />
                        <span>{opt}</span>
                      </label>
                    ))}
...
export default function AssessmentBuilderPage({ params }: PageProps) {
  ...
  const [editingQuestionSection, setEditingQuestionSection] = useState<Section | null>(null);
  ...
  const handleSaveAssessment = async () => {
    ...
    try {
      if (assessment.id && typeof assessment.id === 'string' && assessment.id.includes('-')) {
        // New assessment (UUID), need to create
        const existing = await DatabaseService.getAssessmentByJobId(String(assessment.jobId));
        if (existing) {
          await DatabaseService.updateAssessment(existing.id, assessment);
        } else {
  ...
  const handleAddSection = (sectionData: SectionFormData) => {
    if (!assessment) return;
  ...
  const handleUpdateSection = (sectionData: SectionFormData) => {
  ...
  const handleAddQuestion = (questionData: QuestionFormData, sectionId: string) => {
    if (!assessment) return;
    const updated = assessment.sections.map((s) => {
  ...
  const handleUpdateQuestion = (questionData: QuestionFormData, sectionId: string) => {
  ...
  const headerSubtitle = `${
    assessment.id && typeof assessment.id === 'number' ? 'Edit' : 'Create'
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        setAssessment((s) => (s ? { ...s, title: e.target.value } : s));
                        markDirty();
                      }}
                  ...
                  <div className="space-y-2">
                    ...
                    <textarea
                      ...
                      onChange={(e) => {
                        setAssessment((s) => (s ? { ...s, description: e.target.value } : s));
                        markDirty();
                      }}
                      rows={1}
