...
import { JOB_STATUS, type Job } from '@/lib/types';
...
function JobsList({ jobs, onEdit, onArchive, onDelete, onShowCreate, filters, reordering, onDragEnd, sensors }: JobsListProps) {
  if (jobs.length === 0) {
    return (
...
function SortableJobCard({ job, onEdit, onArchive, onDelete, isReordering }: SortableJobCardProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
