...
import { JOB_STATUS, type Job, type Candidate } from '@/lib/types';
...
function InfoItem({ icon: Icon, label, value, className = '' }: { icon: React.ComponentType<{ className?: string }>; label: string; value: string | undefined; className?: string }) {
...
function EmptyState({ title, subtitle, icon: Icon, children }: { title: string; subtitle: string; icon: React.ComponentType<{ className?: string }>; children?: React.ReactNode }) {
  return (
...
export default function JobDetail({ params }: PageProps) {
  ...
  const stageCounts = useMemo(() => {
    return candidates.reduce((acc: Record<string, number>, c) => {
          ...
          <div className="space-y-6 lg:sticky lg:top-6 h-fit">
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <Link href={`/assessments/create/${id}`} className="block">
